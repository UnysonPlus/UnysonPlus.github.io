...
| [Infinite Scroll Loop](./scroll-loop/index.md) *(Animations tab)* | Loop full-height sections into a seamless, never-ending scroll (Lenis) |
| [Scrollytelling](./scrollytelling/index.md) *(Animations tab)* | Pin one column as a media panel while the other's steps scroll — the media transitions per step (29 styles) |
