...
And the **deterministic Site Converter now walks the same path automatically** — no button, no manual authoring. It recognizes the source's tab widget (extended to a BEM `sc-tabs` / `sc-tabs__list` / `sc-tabs__panels` structure whose buttons resolve their panels by `aria-controls`), and for a **rich** panel (a big verbatim layout — the jukebox menu catalog, detected by length / a grid / multiple sub-headings) it creates one `snippet` CPT per tab holding the panel HTML verbatim inside `.sc-tw` (so the offline Tailwind reproducer paints it — arbitrary `bg-[hsl(…)]` bands *and* semantic `bg-primary`), then sets the tab content to `[snippet id="…"]`. Creation is **idempotent** — keyed by the tab title (`_sc_snippet_key`), so a re-convert updates the same snippet instead of spawning duplicates — and the converter activates the Snippets extension (registering its CPT mid-request) so the path is available on a fresh install. A short FAQ-style panel stays inline; only a full layout becomes a snippet.
