...
- Private functions and classes should be prefixed with:
  ...
  >
  > > [!NOTE]
  > > A **private function** is used somewhere internally. Don't forget to use the [@internal](http://www.phpdoc.org/docs/latest/references/phpdoc/tags/internal.html) tag in your PhpDoc in order to make it clear to other developers that this is a private function. It will also remove the function from your documentation (if you are using an automatic documentation generator)
