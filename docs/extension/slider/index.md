...
To create simple slider type, create a [child extension](/docs/creating-extensions/introduction). In our case the slider type is `demo-slider`, so the child extension directory will be `framework-customizations/extensions/media/extensions/slider/extensions/demo-slider`.
...
1.  **Builder shortcode** - the main slider extension automatically creates a `[slider]` shortcode which is available in [builder](/docs/extension/shortcodes) in the **Media Elements** tab.
