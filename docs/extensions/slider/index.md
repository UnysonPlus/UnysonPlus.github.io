...
To create simple slider type, create a [child extension](/docs/extensions/creating-extensions/introduction). In our case the slider type is `demo-slider`, so the child extension directory will be `framework-customizations/extensions/media/extensions/slider/extensions/demo-slider`.
...
Optionally, if your slider have extra [options](/docs/options/introduction), you can create 2 types of option files within `options/` directory:
...
1.  **Builder shortcode** - the main slider extension automatically creates a `[slider]` shortcode which is available in [builder](/docs/extensions/shortcodes) in the **Media Elements** tab.
