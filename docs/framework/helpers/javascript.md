...
- `fw.ops(properties, value, obj, delimiter)` - same as `fw_aks(...)` from [Php](/docs/framework/helpers/php), but for javascript objects.
...
- `fw.opg(properties, obj, defaultValue, delimiter)` - same as `fw_akg(...)` from [Php](/docs/framework/helpers/php), but for javascript objects.
...
Modal with [options](/docs/framework/options/introduction). Display html generated from a given options array. After the user completes the form and presses "Save", values are available as a javascript object.
