...
```php
$value = $atts['demo_addable_popup_2'];
...
```php
$value = fw_get_db_post_option( get_the_ID(), 'demo_addable_popup_2' );
foreach ( (array) $value as $row ) {
	// each $row is one added item (an array of its sub-option values)
}
```
...
$value = $book['demo_addable_popup_2'];
foreach ( (array) $value as $row ) {
	// each $row is one added item (an array of its sub-option values)
}
```
...
```php
$value = fw_get_db_settings_option( 'demo_addable_popup_2' );
foreach ( (array) $value as $row ) {
	// each $row is one added item (an array of its sub-option values)
}
```
...
`fw_print( fw_get_db_settings_option( 'demo_addable_popup_2' ) )` outputs — the shape of this option type's stored value:
