...
`icon-v2` returns an **array** — read a field by key (the full shape is in *Saved value* below).
...
```php
$value = fw_get_db_post_option( get_the_ID(), 'demo_icon_v2' );
echo '<i class="' . esc_attr( $value['icon-class'] ) . '"></i>';
```
...
```php
$value = fw_get_db_settings_option( 'demo_icon_v2' );
echo '<i class="' . esc_attr( $value['icon-class'] ) . '"></i>';
```

## Saved value

`fw_print( fw_get_db_settings_option( 'demo_icon_v2' ) )` outputs — the shape of this option type's stored value:
