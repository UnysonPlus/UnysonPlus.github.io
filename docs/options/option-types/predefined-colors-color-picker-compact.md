...
```php
$value = $atts['demo_predefined_colors_color_picker_compact_2'];
...
```php
$value = fw_get_db_post_option( get_the_ID(), 'demo_predefined_colors_color_picker_compact_2' );
echo esc_attr( $value['custom'] !== '' ? $value['custom'] : $value['predefined'] ); // custom overrides the preset
```
...
$value = $book['demo_predefined_colors_color_picker_compact_2'];
echo esc_attr( $value['custom'] !== '' ? $value['custom'] : $value['predefined'] ); // custom overrides the preset
```
...
```php
$value = fw_get_db_settings_option( 'demo_predefined_colors_color_picker_compact_2' );
echo esc_attr( $value['custom'] !== '' ? $value['custom'] : $value['predefined'] ); // custom overrides the preset
```

## Saved value

`fw_print( fw_get_db_settings_option( 'demo_predefined_colors_color_picker_compact_2' ) )` outputs — the shape of this option type's stored value:
