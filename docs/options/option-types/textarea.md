...
```php
echo esc_html( $atts['demo_textarea_2'] );
```
...
```php
$value = fw_get_db_post_option( get_the_ID(), 'demo_textarea_2' );
echo esc_html( $value );
```
...
echo esc_html( $book['demo_textarea_2'] );
```
...
```php
$value = fw_get_db_settings_option( 'demo_textarea_2' );
echo esc_html( $value );
```

## Saved value

`fw_print( fw_get_db_settings_option( 'demo_textarea_2' ) )` outputs — the shape of this option type's stored value:
