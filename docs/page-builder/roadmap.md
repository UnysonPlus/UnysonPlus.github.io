...
| **[Clean output](./clean-output-plan.md)** | Every preset Width is a shared `frontend-grid.css` class (twelfths `fw-span-*`, fifths `fw-fifth-*`, content-sizing `fw-w-*`); grid rows are a `fw-grid-N` class with bare cells; and the long tail (a hand-typed value, per-device, min-height, gap) collects into **one** consolidated footer `<style>` keyed to `.fx-<id>` — the Elementor/Bricks/Divi model. No per-element `<style>` blocks. | 🟢 Shipped |
