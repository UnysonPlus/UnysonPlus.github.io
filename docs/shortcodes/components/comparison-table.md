...
- **Rows (Features)** — a repeatable list; each row is a feature line:
  ...
  - **Feature Label** — the feature name (default `Feature`).
  - **Hint** — optional small note under the feature label.
  - **Cell Values** — one line per column, in the same order as the columns. Use `yes` for a check, `no` for a cross, `-` for a dash, or any text for a literal value (e.g. "Up to 5").
