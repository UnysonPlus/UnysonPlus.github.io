...
A list of features should render as a `<ul>` of `<li>`s, not a stack of `<div>`s — then a screen reader announces "list, 5 items" so users know how much there is. UnysonPlus outputs proper list markup, so keep genuinely list-like content in a list. [MDN: the ul element](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/ul) · [Web Dev Basics: Semantic HTML](/learn/semantic-html)
:::
...
- **Default Marker** — image-picker for the fallback marker used by items that have **no** icon (a per-item icon always overrides it): `Checkmark` (default), `Checkmark + icon` (shows both a check and the item's icon), `Numbered`, `Bullets`, or `Icons only` (no fallback decoration). So a list can mix — items with brand icons show their icons, plain items show the checkmark.
