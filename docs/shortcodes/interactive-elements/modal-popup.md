---
title: Modal / Popup
sidebar_position: 30
sidebar_custom_props: { icon: '/img/shortcode-icons/modal-popup.svg' }
---

# Modal / Popup

A trigger (button, text, icon or image) that opens custom content in a modal — centered card,
side drawer or fullscreen. Tabs: **Content**, **Design**, **Styling**, **Animations**,
**Advanced**.

<img src="/img/shortcodes/modal-popup-backend.png" alt="Modal / Popup on the Page Builder canvas" width="936" />

:::tip[💡 Web dev tip: a modal must manage focus]
When a dialog opens, keyboard focus should move into it, stay trapped while it's open, close on <kbd>Esc</kbd>, and return to the trigger afterwards — otherwise keyboard and screen-reader users get lost behind the overlay. Mark it up as `role="dialog"` with `aria-modal="true"`. UnysonPlus' modal does this. [W3C: dialog pattern](https://www.w3.org/WAI/ARIA/apg/patterns/dialog-modal/)
:::

## Content

<img src="/img/shortcodes/modal-popup-content.png" alt="Modal / Popup options panel — Content tab" width="1200" />

| Option | Choices |
| --- | --- |
| **Trigger** | Button · Text link · Icon · Image |
| **Trigger Label** | Default `Open` |
| **Trigger Icon / Image** | Used for the Icon / Image triggers |
| **Modal Title** | Optional |
| **Modal Content** | HTML allowed |

## Design

<img src="/img/shortcodes/modal-popup-design.png" alt="Modal / Popup options panel — Design tab" width="1200" />

| Option | Choices |
| --- | --- |
| **Modal Style** | Center · Left drawer · Right drawer · Fullscreen · … |
| **Size** | Small · Medium · Large (centered card) |
| **Open Animation** | Fade · Zoom · Slide up |
| **Open Automatically** | On/Off (with **Open Delay** in ms) |
| **Close on Overlay Click** | On/Off |

## Styling

<img src="/img/shortcodes/modal-popup-styling.png" alt="Modal / Popup options panel — Styling tab" width="1200" />

**Trigger / Accent Color**, **Overlay Color**, **Modal Background**, **Modal Text**, a **Font
Size Preset**, and **Trigger Margin & Padding**.
