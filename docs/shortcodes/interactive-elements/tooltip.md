---
title: Tooltip
sidebar_position: 36
sidebar_custom_props: { icon: '/img/shortcode-icons/tooltip.svg' }
---

# Tooltip

A trigger (text, icon or button) that reveals a tooltip on hover, focus or click — four
positions and themes. Tabs: **Content**, **Design**, **Styling**, **Animations**, **Advanced**.

<img src="/img/shortcodes/tooltip-backend.png" alt="Tooltip on the Page Builder canvas" width="936" />

:::tip[💡 Web dev tip: tooltips only supplement]
Never put essential information only in a tooltip — many people never hover, and touch users can't. Use it for a helpful extra, make it appear on **keyboard focus** as well as hover, and let it stay on screen long enough to read. [W3C: tooltip pattern](https://www.w3.org/WAI/ARIA/apg/patterns/tooltip/) · [Web Dev Basics: Accessibility](/learn/accessibility)
:::

## Content

<img src="/img/shortcodes/tooltip-content.png" alt="Tooltip options panel — Content tab" width="840" />

| Option | Choices |
| --- | --- |
| **Trigger** | Text (dotted underline) · Button · Icon |
| **Trigger Text** | Default `hover me` |
| **Trigger Icon** | Defaults to `?` if empty |
| **Tooltip Title** | Optional |
| **Tooltip Content** | Basic HTML allowed |

## Design

<img src="/img/shortcodes/tooltip-design.png" alt="Tooltip options panel — Design tab" width="840" />

| Option | Choices |
| --- | --- |
| **Theme** | Dark · Light · Success · Error · Warning · Info |
| **Position** | Top · Right · Bottom · Left (auto-flips on overflow) |
| **Open On** | Hover / focus · Click / tap |
| **Show Arrow** | On/Off |
| **Max Width** | Default `240px` |

## Styling

<img src="/img/shortcodes/tooltip-styling.png" alt="Tooltip options panel — Styling tab" width="840" />

**Tooltip Background**, **Tooltip Text**, **Accent** (button/icon trigger), a **Font Size
Preset**, and **Margin & Padding**.
