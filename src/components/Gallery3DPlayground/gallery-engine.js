	...
	function initReel( el ) {
		...
		function applyReel() {
			...
			for ( var i = 0; i < N; i++ ) {
				...
				var y = R * Math.sin( thr );
				var z = zSign * ( R * Math.cos( thr ) - R );
				c.style.transform = 'translate3d(0,' + y.toFixed( 1 ) + 'px,' + z.toFixed( 1 ) + 'px) rotateX(' + ( zSign * -th ).toFixed( 1 ) + 'deg)';
				var a = Math.abs( th );
		...
		var allowDrag = num( el, 'data-tdg-allowdrag', 1 );
		var momentum = num( el, 'data-tdg-momentum', 1 );
		var dragging = false, py = 0, scrollY = null, target = 0, dwellT = 0;
		...
		if ( allowDrag ) {
			el.style.cursor = 'grab';
			var down = function ( x, y ) { dragging = true; py = y; vel = 0; el.style.cursor = 'grabbing'; };
			var move = function ( x, y ) { if ( ! dragging ) { return; } var dy = y - py; py = y; var d = -dy / Math.max( 1, cardH * 0.9 ); pos += d; vel = d; };
